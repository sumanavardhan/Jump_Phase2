...
---------------------------------------------------------------------------------------
SELECT 
    region_id, COUNT(region_id) AS company_locations
FROM
    countries
GROUP BY region_id
ORDER BY company_locations DESC
LIMIT 1;
---------------------------------------------------------------------------------------
/* 	*
...
SELECT 
    countries.region_id,
    COUNT(*) AS company_locations,
    regions.region_name AS region_with_most_locations
FROM
    countries
        JOIN
...
---------------------------------------------------------------------------------------
SELECT DISTINCT
    country_id, COUNT(country_id) AS Number_Of_Countries
FROM
    locations
WHERE
    state IS NOT NULL
...
WHERE
    products.list_price BETWEEN 100 AND 500;
---------------------------------------------------------------------------------------
/* 	**
...
WHERE
    category_name LIKE 'CPU'
        ...
        AND warehouse_name LIKE 'Toronto';
---------------------------------------------------------------------------------------
/* 	**
...
---------------------------------------------------------------------------------------
SELECT 
    product_name, AVG(list_price) AS Avg_List_Price
FROM
    products
...
SELECT 
    product_categories.category_name,
    ROUND(AVG(list_price), 2) AS Category_Avg_List_Price
FROM
    product_categories
        JOIN
    products ON product_categories.category_id = products.category_id
...
SELECT 
    product_categories.category_name,
    products.list_price,
    COUNT(products.category_id) AS Count_Of_Groups
FROM
    product_categories
        JOIN
    products ON product_categories.category_id = products.category_id
GROUP BY product_categories.category_name , products.list_price
HAVING Count_Of_Groups != 1
ORDER BY category_name , Count_Of_Groups DESC;
---------------------------------------------------------------------------------------
/* 	**
...
set @rowindex =-1;
SELECT 
    (@rowindex:=@rowindex + 1) AS rowindex, unit_price AS Price
FROM
    order_items
ORDER BY unit_price;
---------------------------------------------------------------------------------------
...
SET @rowindex := -1;
SELECT 
    (@rowindex:=@rowindex + 1) AS rowindex, list_price
FROM
    products
ORDER BY list_price;
---------------------------------------------------------------------------------------  
...
SET @rowindex := -1;
SELECT 
    AVG(list_price) AS median
FROM
    (SELECT 
        (@rowindex:=@rowindex + 1) AS rowindex, list_price
    FROM
        products
    ORDER BY list_price) p
WHERE
    p.rowindex IN (FLOOR(@rowindex / 2) , CEIL(@rowindex / 2));
---------------------------------------------------------------------------------------
/* 	*****
